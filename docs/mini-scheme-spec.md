...
1. 本仕様 (`mini-scheme-spec.md`) で確定できる場合、R5RS と異なっていても本仕様を優先する
2. 本仕様だけでは確定しない場合、R5RS の仕様に倣う
3. いずれでも確定しない場合、実装任意とする（実装時にコメント等で方針を記録すること）
...
| 型          | 構文                             | 備考                      |
|------------|--------------------------------|-------------------------|
| **Num**    | `42`, `-3`, `1/2`, `1.5`, `-2.5` | 10進整数、有理数、小数をサポート      |
| **Bool**   | `#t`, `#f`                     |                         |
...
整数・有理数同士の `/` は exact な有理数を返す。結果の分母が 1 の場合は整数として表示する。どちらかの引数が浮動小数点数の場合は浮動小数点数を返す。
